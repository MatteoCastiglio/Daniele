...
import daniele.state.PawnPosition;
...
public class AIGameSingleThread implements AIGame {
    ...
    private boolean useTraspositionTable = false;
    ...
    public AIGameSingleThread(HeuristicFunction h, long maxTime, MinMaxPrinter printer,  boolean useDrawCondition, boolean orderingOptimization) {
        this(h, maxTime, printer,  useDrawCondition);
        this.orderingOptimization = orderingOptimization;

    }

    public AIGameSingleThread(HeuristicFunction h, long maxTime, MinMaxPrinter printer, boolean useDrawCondition) {
        this(h, maxTime, printer) ;
        this.useDrawCondition = useDrawCondition;

    }

    public AIGameSingleThread(HeuristicFunction h, long maxTime, MinMaxPrinter printer) {        //if maxTime = -1 there is no time limit
        this.heuristic = h;
...
;
    ...
    //TO-DO refactoring code in NEGAMAX style
    public DanieleAction chooseBestMove(int startingDepth, int maxDepth, ITablutState ts, Set<String> pastStates) {
        ...
        //debug     long startTime = System.nanoTime();



        //iterative deeping with alpha-beta prunning 
        ...
        if (ts.getState().getTurn().equals(Turn.WHITE)) {
            ...
            for (int depth = startingDepth; depth <= maxDepth; depth++) {
                ...
                for (DanieleAction m : moves) {
                    ...
                    List<PawnPosition> l = ts.trasformState(m);

                    if (useDrawCondition && pastStates.contains(ts.getState().toLinearString())) {    //a move has been found that leads to a draw
                        v = DRAW_VALUE_WHITE;
                    } else {
                        v = MinValue(depth - 1, alpha, Double.POSITIVE_INFINITY, ts, printer);
                    }
            ...
            printer.printDecision(bestActionLastDepth,maxDepth);
            return bestActionLastDepth;
                ...
                for (DanieleAction m : moves) {
                   ...
                   List<PawnPosition>  l= ts.trasformState(m);
                    ...
                    if (useDrawCondition && pastStates.contains(ts.getState().toLinearString())) {    //a move has been found that leads to a draw
                        ...
                        v = MaxValue(depth - 1, Double.NEGATIVE_INFINITY, beta, ts, printer);
                    }
    ...
    private double MaxValue(int depth, double alpha, double beta, ITablutState state, MinMaxPrinter printer) {
        ...
        if (cutoff(depth, state)) {
            return heuristic.computeValue(state) - depth;
        }
        ...
        for (int i = 0, movesSize = moves.size(); i < movesSize; i++) {
            ...
            List<PawnPosition> p =state.trasformState(m);        //instead of:	ITablutState childState = state.getChildState(m);
