...
public class FinalGame {
	...
	private static boolean playGame(double[] bestAgent, double[] agent, int numberOfMatch) throws IOException {
		// ritorna vero se l'agent (il nuovo esaminato - NEW) � migliore del best agent (il migliore per il momento - OLD) ; falso altrimenti

		for (int i = 0; i < DifferentialEvolution.nGames; i++) {

			ProcessBuilder server_pb = new ProcessBuilder("java", "-cp", "lib/gson-2.2.2.jar;bin", "Daniele.training.FinalServer");        // ; per windows	, 	: per mac
		...
		if (stats[1] == stats[0] && (stats[4] > stats[3])) {
			System.out.println("Agent updated because more moves: " + stats[3] + " vs " + stats[4]); //ha vinto entrabe le partite il WHITE, e il BLACK � stato pi� resistente
			return true;
		}
		return false;
	}
