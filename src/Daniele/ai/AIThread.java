...
public class AIThread extends Thread {
    ...
    private  DanieleAction result=null;
    ...
    private List<ITablutState> states;
    ...
    public DanieleAction getResult() {
        return result;
    }
    ...
    public void run() {
        ...
        if(states.get(0).getState().getTurn().equals(it.unibo.ai.didattica.competition.tablut.domain.State.Turn.BLACK)) {    //MAX player
            for (ITablutState ts : states) {
                    ...
                    for (DanieleAction m : moves) {
                        ...
                        if (useDrawCondition && pastStates.contains(ts.getState().toLinearString())) {
                            ...
                            if (timeOver) {
                                ...
                                System.out.println("TIMEOVER: time=" + time + ", depth = " + depth + " -  bestActionLastDepth: " + result.toString());
                                // -----
                                return;
                            }
                            ...
                            if (v > value) { //pruning
                                ...
                                if (depth == startingDepth) {    //potrebbe servire nel caso si decidesse di partire con una profondità già abbastanza notevole -
                                    result = m;    //in modo che se scade il timer si avrà una già una bestActionLastDepth prima di finire il for-moves_same_depth
                ...
                timer.cancel();
                time = (double) (System.nanoTime() - startTime) / 1_000_000_000.0;
                System.out.println("Time=" + time + ", depth = " + maxDepth + " - bestActionLastDepth = " + result.toString());
                return;
            }
        }
        else if(states.get(0).getState().getTurn().equals(it.unibo.ai.didattica.competition.tablut.domain.State.Turn.WHITE)) {	//MIN player
            for (ITablutState ts : states) {
                LinkedList<DanieleAction> moves = (LinkedList<DanieleAction>) ts.getAllLegalMoves();
     ...
     * @return
     */
    protected double MinValue(int depth, double alpha, double beta, ITablutState state,MinMaxPrinter printer) {
    ...
    private boolean cutoff(int depth, ITablutState state) {
        ...
        return depth <= 0
                || (!state.getState().getTurn().equals(it.unibo.ai.didattica.competition.tablut.domain.State.Turn.WHITE) && !state.getState().getTurn().equals(it.unibo.ai.didattica.competition.tablut.domain.State.Turn.BLACK))
                || timeOver;
    }
